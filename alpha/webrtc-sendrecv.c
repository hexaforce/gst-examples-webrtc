...
static GOptionEntry entries[] = {
  ...
  {"server", 0, 0, G_OPTION_ARG_STRING, &server_url,
      "Signaling server to connect to", "URL"},
  ...
  {"custom-ice", 0, 0, G_OPTION_ARG_NONE, &custom_ice, "Use a custom ice agent",
      NULL},
  {NULL},
};
...
webrtcbin_get_stats (GstElement * webrtcbin)
{
  ...
  promise = gst_promise_new_with_change_func ((GstPromiseChangeFunc)
      on_webrtcbin_get_stats, webrtcbin, NULL);
...
start_pipeline (gboolean create_offer, guint opus_pt, guint vp8_pt)
{
  ...
  if (custom_ice) {
    ...
    webrtc1 =
        gst_element_factory_make_full ("webrtcbin", "name", "sendrecv",
...
check_plugins (void)
{
  ...
  const gchar *needed[] =
      { "opus", "vpx", "nice", "webrtc", "dtls", "srtp", "rtpmanager",
